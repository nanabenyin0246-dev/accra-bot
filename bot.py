...
def build_ai_providers():
    ...
    if SAMBANOVA_KEY:
        AI_PROVIDERS.append({"name":"sambanova","url":"https://api.sambanova.ai/v1/chat/completions",
            "model":"Meta-Llama-3.3-70B-Instruct","headers":{"Authorization":f"Bearer {SAMBANOVA_KEY}","Content-Type":"application/json"},"max_tokens":300})
...
def trade_existing_assets(strategy, cfg):
    try:
        ...
        if best["symbol"] != worst["symbol"] and best["score"] > 40 and worst["score"] < 25:
            ...
            sell_qty = round(worst["qty"] * 0.95, get_binance_lot_size(worst["symbol"]))
        ...
        for t in scored:
            ...
            try:
                ...
                if should_sell:
                    prec = get_binance_lot_size(sym)
                    qty = round(t["qty"] * 0.95, prec)
                    if qty * t["price"] >= 2:
...
def main():
    ...
    while True:
        ...
        # This sleep block used to be dedented one level too far, placing
        # it OUTSIDE this `while True:` loop -- unreachable dead code.
        # The bot has never actually slept between cycles; it looped
        # run_cycle() back-to-back with zero pause, which is the real
        # reason cycles ran every ~8s regardless of SLEEP_SECS and is
        # what was tripping GitHub's rate limit on every Gist status
        # push. Re-indented so it actually runs each iteration.
